...
def run(
        ...
        max_det=1024,  # maximum detections per image
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    w_cm = w * 0.2531321181
                    h_cm = h * 0.3725490196
                    #print(f'Object ID: {object_id}, Label: {label}, Confidence: {confidence_str}, Coordinates: x={x_cm}, y={y_cm}, w={w_cm}, h={h_cm}')

                    start_time = datetime.datetime.now()
                    insert_data_into_mysql(object_id, label, confidence, x_cm, y_cm, w_cm, h_cm)
                    end_time = datetime.datetime.now()
                    communication_time = end_time - start_time
                    communication_time = communication_time.microseconds / 1000
...
def parse_opt():
    ...
    parser.add_argument('--max-det', type=int, default=1024, help='maximum detections per image')
